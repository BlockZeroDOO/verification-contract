...
namespace verification_retail_tables {
...
struct [[eosio::table("rtlreceipts")]] payment_receipt_row {
    uint64_t receipt_id;
    uint8_t mode;
    name payer;
    name submitter;
...
using payment_receipt_table = multi_index<
    "rtlreceipts"_n,
    payment_receipt_row,
    indexed_by<"byrequest"_n, const_mem_fun<payment_receipt_row, checksum256, &payment_receipt_row::byrequest>>,
    indexed_by<"bypayer"_n, const_mem_fun<payment_receipt_row, uint64_t, &payment_receipt_row::bypayer>>
>;

struct [[eosio::table("rtlcounters")]] retail_counter_state {
    uint64_t next_token_id = 1;
    uint64_t next_tariff_id = 1;
    uint64_t next_receipt_id = 1;
};
