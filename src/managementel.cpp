...
void managementel::addwhuser(const name& account, const string& note) {
...
void managementel::rmwhuser(const name& account) {
    require_auth(get_self());
...
void managementel::addnporg(const name& account, const string& note) {
    require_auth(get_self());
    check(is_account(account), "account does not exist");
...
void managementel::rmnporg(const name& account) {
    require_auth(get_self());
...
void managementel::rmpaytoken(const name& token_contract, const symbol& token_symbol) {
    require_auth(get_self());
...
void managementel::submitfree(
    ...
    record_proof(submitter, parsed_hash, canonicalization_profile, client_reference);
}

void managementel::setfreecfg(
    bool enabled,
...
void managementel::ontransfer(const name& from, const name& to, const asset& quantity, const string& memo) {
    ...
    record_proof(from, parsed_hash, canonicalization_profile, client_reference);
}

asset managementel::quote(
    const name& account,
    const name& token_contract,
...
bool managementel::iswhuser(const name& account) const {
    wholesale_table wholesale(get_self(), get_self().value);
    return wholesale.find(account.value) != wholesale.end();
}

bool managementel::isnporg(const name& account) const {
    nonprofit_table nonprofits(get_self(), get_self().value);
    return nonprofits.find(account.value) != nonprofits.end();
}

managementel::free_policy managementel::get_free_policy() const {
    free_policy_singleton policy_store(get_self(), get_self().value);
    check(policy_store.exists(), "free submission config is not set");
    return policy_store.get();
}

time_point_sec managementel::current_day_start(const time_point_sec& timestamp) const {
...
uint128_t managementel::make_payment_key(const name& token_contract, const symbol_code& token_symbol) const {
    return (static_cast<uint128_t>(token_contract.value) << 64) | token_symbol.raw();
}

managementel::payment_token managementel::get_payment_token(
    const name& token_contract,
...
void managementel::consume_free_allowance(const name& submitter) {
    free_policy_singleton policy_store(get_self(), get_self().value);
...
managementel::pricing_decision managementel::resolve_pricing(
    const name& account,
    const name& token_contract,
    const symbol& token_symbol
) const {
...
asset managementel::resolve_price(
    const name& account,
    const name& token_contract,
    const symbol& token_symbol
) const {
...
managementel::pricing_decision managementel::resolve_paid_pricing(
    const name& account,
    const name& token_contract,
    const symbol& token_symbol
) const {
...
managementel::pricing_decision managementel::resolve_token_pricing(
    const name& token_contract,
    const symbol& token_symbol,
...
checksum256 managementel::parse_hash(const string& hex) const {
...
void managementel::validate_client_reference(const string& client_reference) const {
...
void managementel::validate_printable_ascii_text(
    const string& value,
    uint32_t max_length,
...
void managementel::validate_payment_price(const asset& price, const char* field_name) const {
...
void managementel::validate_nonnegative_asset(const asset& quantity, const char* field_name) const {
...
std::tuple<string, string, string, string> managementel::parse_payment_memo(const string& memo) const {
...
uint8_t managementel::from_hex(char c) const {
    if (c >= '0' && c <= '9') {
...
void managementel::validate_new_request(const name& submitter, const string& client_reference) const {
    verification_proof_table proofs(verification_account, verification_account.value);
...
void managementel::record_proof(
    const name& submitter,
    ...
    action(
        ...
        "record"_n,
        std::make_tuple(submitter, object_hash, canonicalization_profile, client_reference)
    ).send();
}

void managementel::validate_text(
    const string& value,
    uint32_t max_length,
    const char* field_name,
    bool allow_empty
...
extern "C" {
    ...
    void apply(uint64_t receiver, uint64_t code, uint64_t action) {
        ...
        if (action == "transfer"_n.value) {
            eosio::execute_action(name(receiver), name(code), &managementel::ontransfer);
        }
    }
}
